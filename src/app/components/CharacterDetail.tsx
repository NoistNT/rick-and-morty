...
export default function CharacterDetail({ character }: { character: CharacterDetail }) {
  ...
  return (
    <section className="m-auto flex min-h-screen max-w-max flex-col items-center justify-center gap-3">
      <div className="w-full">
        <button
          className="w-full rounded-md border-2 border-neutral-800 bg-slate-800/80 px-4 py-1.5 text-center transition-colors duration-150 ease-in-out hover:border-white/10 hover:bg-slate-800 hover:bg-opacity-90"
          type="button"
          onClick={() => router.back()}
        >
          Go Back
      ...
      <article className="max-w-[300px] rounded-md border-2 border-neutral-800 bg-slate-900 bg-opacity-80 text-center">
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <div className="flex w-full flex-col gap-4 px-2 pb-3">
            ...
            <div className="flex justify-between rounded-md bg-neutral-700/50 px-2 py-1">
              Type:
              <h2 className={character.type === '' ? 'text-orange-500' : 'text-gray-400'}>
            ...
            <div className="flex justify-between rounded-md bg-neutral-700/50 px-2 py-1">
              ...
              <h2
                className={
                  character.origin.name === 'unknown' ? 'text-orange-500' : 'text-gray-400'
                }
              >
                {character.origin.name[0].toUpperCase() + character.origin.name.slice(1)}
              </h2>
            </div>
            <div className="flex justify-between rounded-md bg-neutral-600/20 px-2 py-1">
              Location:
              <h2
                className={
                  character.location.name === 'unknown' ? 'text-orange-500' : 'text-gray-400'
                }
              >
                {character.location.name[0].toUpperCase() + character.location.name.slice(1)}
              </h2>
            </div>
          </div>
        </div>
